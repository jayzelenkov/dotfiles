...
ICON_PLAY=$(printf '\xef\x81\x8b')   # U+F04B nf-fa-play
ICON_PAUSE=$(printf '\xef\x81\x8c')  # U+F04C nf-fa-pause
...
update() {
  ...
  if [ "$(echo "$INFO" | jq -r '.["Player State"]')" = "Playing" ]; then
    PLAYING=0
    TRACK="$(echo "$INFO" | jq -r .Name | sed 's/\(.\{24\}\).*/\1.../')"
    ARTIST="$(echo "$INFO" | jq -r .Artist | sed 's/\(.\{24\}\).*/\1.../')"
    ALBUM="$(echo "$INFO" | jq -r .Album | sed 's/\(.\{24\}\).*/\1.../')"
    SHUFFLE=$(osascript -e 'tell application "Spotify" to get shuffling')
    REPEAT=$(osascript -e 'tell application "Spotify" to get repeating')
  ...
  if [ $PLAYING -eq 0 ]; then
    ...
    args+=(--set spotify.play icon="$ICON_PAUSE"
    ...
    args+=(--set spotify.anchor drawing=off popup.drawing=off
           --set spotify.play   icon="$ICON_PLAY")
  fi
  sketchybar -m "${args[@]}"
}
